...
- **FR-INST-22** (S) The IDE SHALL present a persistent sidebar surface (an activity-bar view container) as the participant's primary view of the study, showing session state and, for all four legs (static metrics, beh… — _⬜_
...
- **FR-DASH-13** (S) The grounded toggle catalog (FR-DASH-11) SHALL span all four instrument legs, not only `tern` and `agentCapture`: every leg the protocol can enable SHALL expose its participant-visible capture deci… — _⬜_
...
- **FR-OPS-8** (C) The IDE extension SHALL be distributable as a published artifact: a real publisher identity, icon, README/CHANGELOG/LICENSE, declared activation events, and a `vsce`-packaged `.vsix` produced on re… — _⬜_
