...
- **FR-INST-20** (M) The framework SHALL let a participant's IDE enroll into a study by redeeming one pairing token (a connection string encoding middleware URL + token) that resolves its identity (participantId, condi… — _✅ mint/redeem/server-stamp verified via pytest; extension pairing state machine (13 transition tests) + `vscode://…/pair` URI handler; platform enrollment table with copy-link + deep-link + live polling_
- **FR-INST-21** (M) The middleware SHALL serve a capture config derived deterministically from the protocol's instruments block; the IDE SHALL apply it at pair and at each session start behind a capture pre-flight; a… — _✅ derive/version/apply-at-boundary pytest + `node:test` verified, incl. a wall-6 lifecycle test (apply → mid-session-refuse → next-boundary-applies); re-pull at each session boundary; `captureConfigVersion` enforced server- and client-side_
- **FR-INST-22** (S) The IDE SHALL present a persistent sidebar surface (an activity-bar view container) as the participant's primary view of the study, showing session state and, for all four legs (static metrics, beh… — _🔶 Phase 25 Slices B+C built — `src/core/legs.ts` (portable, 12 `node:test` cases) + `src/vscode/sidebar.ts` (Session / What is captured / Your data tree views, activity-bar container, consent banner). Remaining: the Extension Dev Host walkthrough (owner-run) and written/mirrored event counts not yet plumbed from the recorder_
...
- **FR-ING-7** (M) The middleware SHALL mint, list, and revoke pairing tokens (study-scoped, ethics-gated, role-gated) and verify them on redemption, issuing a short-lived session credential; credentialed ingest SHAL… — _✅ mint/list/revoke/redeem, server-stamp, never-block-never-drop, and the token/credential-never-persists grep test all pytest + live-verified_
...
- **FR-DASH-10** (S) The platform SHALL provide an enrollment surface in the study workspace: mint pairing tokens (batch/single, pick grain) as copy-links with live status (unredeemed/paired/streaming) and revoke, role… — _✅ mint/list/revoke + streaming status + per-row capture-config visibility + copy-link + deep-link "Open in VS Code" + 15s live polling; NFR-12 browser evidence pending manual walkthrough_
...
- **FR-DASH-13** (S) The grounded toggle catalog (FR-DASH-11) SHALL span all four instrument legs, not only `tern` and `agentCapture`: every leg the protocol can enable SHALL expose its participant-visible capture deci… — _🔶 Phase 25 Slice A built — catalog spans all four legs; `leg_summary()` returns all four with enabled/disabled/unavailable, rides on the capture config so the IDE renders one catalog. Remaining: the platform console grouping its toggles by leg_
...
- **FR-LIT-10** (S) The study's knowledge SHALL be explorable as a living literature view: the citation graph animated as a responsive constellation that reacts to the conversation (papers glow when cited in a turn, d… — _🔶 the canvas itself (`Constellation.tsx`): degree-sized nodes, near-invisible edges that reveal their kind colour only in a hovered/focused neighbourhood (precomputed adjacency, keyboard parity, `Escape` clears), labels revealed by zoom/focus/selection instead of sitting on permanently, idle drift, and reduced-motion parity (the deterministic `layoutGraph` seed alone, fully interactive, nothing skipped). Still open: cite-pulses tied to conversation citations, arrival streaks, cluster halos, gap halos, lasso scope, and the scoped-RAG/guidance-intent loop into the assistant (`fr-lit.md` §3 items 2-4, fit F10.1-F10.4)_
...
- **FR-PLAT-4** (S) The platform SHALL present a public hero page: what it does, a self-contained animated showcase of the core loop, and sign-up - readable by a lay researcher per NFR-11. — _✅ (built; the animated constellation + deterministic core-loop showcase replaced the embedded live-LLM demo, which was unreliable)_
...
- **FR-PLAT-6** (C) A study open in more than one place SHALL show who else is viewing it and SHALL push a change notice (new turn, decided move, applied draft) to those viewers, who then re-read through the ordinary… — _✅_
...
- **FR-CONV-5** (S) Researcher feedback SHALL be capturable in-conversation as structured findings feeding the retrospective and in-platform agents (FR-META-1/2/3) as inert, human-approved proposals; cross-project lea… — _- built (feedback→findings→inert proposal + aggregates-only shapes green; grep-the-output enforced; UI built + gated), then removed 2026-08-06: the "Platform findings" tab wasn't earning its keep, so the whole capture-to-proposal pipeline (conversation flag affordance, offline store, `/platform/findings`, `/platform/aggregate`, `/platform/retrospective`) was withdrawn. FR-META-1/2 (the general findings log + retrospective) are unaffected — only the FR-CONV-5 extension of them is gone._
...
- **FR-CONV-9** (S) The design conversation SHALL adapt to the researcher's declared experience — student, new researcher, experienced researcher, or industry practitioner — in register, in how much methodology it exp… — _✅_
- **FR-CONV-10** (M) The platform SHALL NOT propose a design shape (a `choose-template` move) until the conversation has established enough of the study — who takes part, what they do, what is compared, what is measure… — _✅_
...
- **FR-CUR-2** (S) A GitHub mining adapter SHALL import repositories, pull requests, commits, and issues via the GitHub API into curated-dataset rows - rate-limited, cached, resumable, degrading gracefully (NFR-4 ext… — _❌ Removed 2026-08-06 — GitHub adapter retired, no UI/HTTP consumer remained (owner direction: drop GitHub mining entirely)_
