...
Two things have to be true for the dial to be a real control rather than a
label on a prompt:

1. it moves BOTH levers — the register the assistant speaks in and how much it
   proposes unprompted — and
2. the "propose nothing" end is *enforced* on the moves themselves, not merely
   requested of the model, because a prompt can be ignored and a filter
   cannot.

The invariant the dial must never break: the method does not change with it.
The same designs, the same statistics, the same honesty about grounding. Only
the volume changes.
"""
...
def test_every_level_names_a_register_and_an_initiative():
    """Both levers, for every stop — a level that only changed the register
    would make the dial a duplicate of the profile setting in Settings."""
    for key, spec in elicitation.STEER_LEVELS.items():
        ...
        # The register a level implies is exactly what steer_profile reports.
        assert elicitation.steer_profile(key) == spec["profile"]


def test_an_unknown_or_absent_level_falls_back_rather_than_going_silent():
    """An older client, or an agent posting a turn without the field, must
    still get a full instruction — never an empty one."""
...
def test_checks_drops_proposals_but_never_drops_a_caution():
    """The enforcement half. At `checks` the assistant answers and warns; it
    does not propose. A caution is not a proposal — it is the one thing this
    level exists to keep."""
    moves = (
...
def test_the_directive_carries_both_levers_into_the_prompt():
    """The model is told who it is speaking to AND how much to drive, in that
    order. A directive missing either half silently reverts that lever to the
    model's own habit."""
...
def test_the_dial_never_changes_the_method():
    """The guardrail behind the whole feature: no level may promise different
    designs, weaker statistics, or softer grounding. Turning steer down buys
    a quieter colleague, never a less rigorous one."""
