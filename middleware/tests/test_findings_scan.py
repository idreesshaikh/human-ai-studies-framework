...
def test_requires_fail_finding_round_trips(client):
    """
    What the analysis runner posts on a failed requires-check (FR-ANA-2): a
    requires-fail finding, which the retrospective later cites.
    """
    res = client.post(
        "/findings",
        json={
...
def test_gap_and_requires_fail_yield_two_findings(client):
    """
    Acceptance: a fake seq gap + a failed requires-check yield two findings rows - and
    the status doc surfaces the gap so the platform cards it, while /findings surfaces
    the requires-fail.
    """
    client.post("/ingest/events", json=[event(i) for i in (0, 1, 3)])
    client.post("/studies/pilot-2026/findings/scan")
