...
def test_dataset_summary_is_aggregates_only(client):
    """
    FR-ETH-4: the summary tool exposes counts and metric means/medians - never a
    participantId, sessionId, or raw payload value.
    """
    from middleware.db import make_session_factory
    ...
    assert "cognitive_complexity" in summary
    assert "P01" not in summary
    assert "S1" not in summary
    assert "212" not in summary
