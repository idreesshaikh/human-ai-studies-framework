...
(cognitive + behavioral legs on one timeline, MP-06), posts the sample
...
#: dashboard acceptance walk uploads the ethics artifacts via the UI.
DESIGN_ARTIFACTS = {
...
def _upload(server: str, filename: str, content: bytes) -> dict:
    ...
    req = urllib.request.Request(
        f"{server}/ingest/files", data=body, method="POST",
...
def main() -> int:
    ...
    parser.add_argument("--events", type=Path,
                        default=SAMPLE_DIR / "sample-session.jsonl")
    parser.add_argument("--events2", type=Path,
                        default=SAMPLE_DIR / "sample-session-2.jsonl",
                        help="behavioral demo session (MP-06 timeline)")
    parser.add_argument("--events3", type=Path,
    ...
    parser.add_argument("--metrics", type=Path,
                        default=SAMPLE_DIR / "sample-metrics.jsonl")
    args = parser.parse_args()

    try:
        health = _call("GET", f"{args.server}/health")
    except (urllib.error.URLError, OSError) as exc:
        print(f"error: middleware not reachable at {args.server}: {exc}",
              file=sys.stderr)
        return 1
    ...
    print(f"events: {first['inserted']} inserted, then replayed -> "
    ...
    if args.events2.exists():
        ...
        print(f"behavioral session {behavior[0]['sessionId']}: "
    ...
    if args.events3.exists():
        ...
        print(f"unassisted session {unassisted[0]['sessionId']}: "
    ...
    try:
        ...
        print(f"lifecycle: current phase {lifecycle['currentPhase']!r} "
    ...
    for g in gaps["gaps"]:
        print(f"  missing {g['missing']} event(s) between seq "
