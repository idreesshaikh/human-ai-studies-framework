...
| FR-PLAT-4 | S7; NFR-11 | hero page + `/demo` API pointer (SPA client route `/showcase`, deliberately different — see `app.py`'s SPA-fallback comment) | ✅ server-seeded demo project: `_ensure_demo_project` creates/repoints the demo `Project` onto the boot study whenever `MIDDLEWARE_SEED_ON_START=1` (`test_demo_project.py`); a self-hosted deployment without that flag never exposes its real study as the public demo |
