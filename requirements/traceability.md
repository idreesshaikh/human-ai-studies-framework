...
| FR-PLAT-4 | S7; NFR-11 | hero page | ⬜ the shared read-only public-demo pointer (`/demo` API + `/showcase` route + `_ensure_demo_project` seeding) was built 2026-07-21, then removed the same day on owner review (the scripted/no-value-add read-only walkthrough didn't earn its keep); the hero's live scripted preview (`Hero.tsx`, `stubOnly`) still stands on its own without it |
