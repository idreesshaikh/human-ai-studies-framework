...
| FR-ETH-4 | M | Knowledge-assistant calls to the ~~Claude API~~ LLM provider *(rev 2, 2026-07-16: Gemini/Mistral per D32 - the boundary is provider-independent)* SHALL send only paper text, protocol content, and aggregate dataset summaries - never row-level participant events. | NFR-5 boundary for the one cloud-touching feature. | ✅ MP-10; provider swapped under D32, boundary tests unchanged |
