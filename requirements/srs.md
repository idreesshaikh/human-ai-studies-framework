...
(current vision: `docs/VISION.md`; v1 sprint vision preserved at
`docs/archive/roadmap/00-VISION.md`). IDs are stable; MoSCoW priorities
define the cut line per milestone - for the v1 families: **Must = the one-week production slice. Should = built if
...
interaction is a **design conversation** - experiments are built from
conversations, study ideas and instrumentation evolve on the fly, and
every idea is grounded in the paper corpus; paper-derived study templates
prescribe the statistical formulation; curated datasets (GitHub mining)
stand beside live capture; the platform itself evolves from researcher
feedback; agent-readable metadata throughout; a product-grade UI (D34
React + shadcn/ui surface, D35 Claude-driven design, NFR-12). Vision:
`docs/VISION.md`; elicitations logged in `traceability.md` §3. The five
v2 families (FR-PLAT, FR-TPL, FR-CONV, FR-CUR, FR-AGF) carry priorities
against the **v2 milestone** (thesis-final deliverable); the completed
sprint's cut line and statuses are untouched (golden rule 6). Each v2
family's one-line rows below index a **detailed specification** in
`specs/` - data models, fit criteria, degradation posture, paper
grounding; the row wins on conflict.
...
The differentiator: published methodology becomes executable. Grounded in
the 58-paper corpus (`docs/papers/README.md`); generalizes RQ-F3 from
"replicate our study" to "replicate the literature's designs". Detailed
spec: `specs/fr-tpl.md` (template object model, seed templates, the
statistical-plan decision structure).
...
| FR-TPL-2 | M | Every template SHALL bind a **statistical plan**: the exact tests, effect sizes, corrections, and per-cell-n rules its design requires (NFR-8 by construction), emitted into the instantiated protocol's analysis plan so the researcher gets correct statistics without deriving them. | Owner elicitation 2026-07-17: what frustrates researchers is "the algorithms and statistical formulation they should use" - this is the platform's core promise. Anchored in Baltes et al. guidelines (arXiv:2508.15503). | ⬜ |
